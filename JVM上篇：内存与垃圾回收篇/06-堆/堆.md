...
![image-20240614182805074](https://raw.githubusercontent.com/EXsYang/PicGo-images-hosting/main/images/image-20240614182805074.png)
...
**注意：在伊甸园区(Eden)满了的时候会触发YGC/Minor GC，但在幸存者区(Survivor)满了的时候却不会触发YGC/Minor GC,但是并不意味着幸存者区不存在垃圾回收,即幸存者区存在垃圾回收，这是两回事儿!!!**

**当伊甸园区(Eden)满了的时候会触发YGC/Minor GC,将伊甸园区(Eden)和幸存者区(Survivor)一起进行垃圾回收**
...
1. 直接往Eden区放，放不下(超大对象)，会尝试直接放到Old区，再判断Old是否放得下？否->FGC/Major GC->再判断Old是否放得下？放得下就放,放不下OOM
2. Eden区往Survivor区放【Eden区满了,YGC后Eden区往Survivor区放】，如果放不下则直接晋升老年代Old区
...
简要总结为什么对象实例在Oracle HotSpot JVM中主要仍然创建在堆上：

1. **技术复杂性和稳定性**：栈上分配技术虽然理论上可行，但实现复杂，可能影响虚拟机的稳定性和通用性。

2. **逃逸分析的限制**：虽然逃逸分析可以识别不逃逸的对象，但由于逃逸分析本身的性能开销以及精确度的限制，HotSpot JVM选择更保守的策略，以保证性能和兼容性。

3. **优化选择**：HotSpot JVM更倾向于使用标量替换和锁消除等其他形式的优化，这些不直接涉及栈上分配。

因此，在实际应用中，**对象的实例化过程通常仍然在堆上进行**，以避免潜在的风险和复杂性，同时确保JVM的高效和稳定运行。
